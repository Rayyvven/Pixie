...
func _ready() -> void:
	if "Tile4" in LevelController.LevelData["Level Data"]:
		print("hi")
		LevelController.Loading = true
		print("Loading:", LevelController.Loading)
		LoadFromLevelController()
	...
	if event is InputEventMouseButton and event.is_pressed():
		if not get_global_mouse_position().x >= 1000 or EditorOptionsGlobal.Shown == true or get_viewport().get_mouse_position().y >= 1700 or get_viewport().get_mouse_position().y <= 150 and get_viewport().get_mouse_position().x <= 150 or EditorOptionsGlobal.Shown:
			if event.button_index == MouseButton.MOUSE_BUTTON_LEFT:
				...
				Input.vibrate_handheld(100, 0.05)## For Mobile: vibrates the phone a little.

func LoadFromLevelController() -> void:
	...
	var songfile = FileAccess.open(LevelController.SavedLevelData["Level Info"]["Song"], FileAccess.READ) ## This is stupid
	...
	while "Tile" + str(i) in LevelController.SavedLevelData["Level Data"]:
		if LevelController.SavedLevelData["Level Data"]["Tile" + str(i)] != ["Null"]:
